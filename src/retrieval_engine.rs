...
use crate::{global_config::GlobalConfig, shared_log::{agent_recorder::AgentRecorder, traits::SharedLog, user_embedding_model::SlimUserEmbeddingInput}};
...
impl RetrievalEngine{
    pub fn new(global_config: GlobalConfig) -> Self {
        ...
        RetrievalEngine { shared_log: recorder }
    }
    ...
    pub fn get_cached_agent_response(
        ...
        match self.shared_log.find_similar_user_event(content.clone()){
        ...
        match agent_response{
            Ok(response) => {
                tracing::info!("Cache hit for user query {}: {}", content.clone(), response.get_content());
