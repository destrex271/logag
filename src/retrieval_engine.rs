...
use crate::{global_config::GlobalConfig, language_handler::service::LanguageService, shared_log::{agent_recorder::AgentRecorder, traits::SharedLog, user_embedding_model::SlimUserEmbeddingInput}};
...
impl RetrievalEngine{
    ...
    pub fn get_cached_agent_response(
        ...
        match self.shared_log.find_similar_user_event(formatted_content.clone()){
            Ok(response) => user_event_ref = response,
            Err(err) => return format!("unable to find similar user input: {:?}", err),
        };

        let agent_response = self.shared_log.fetch_ai_response_for_user_event(user_event_ref.user_event_id);
        match agent_response{
            Ok(response) => {
                tracing::info!("Cache hit for user query {}: {}", formatted_content.clone(), response.get_content());
